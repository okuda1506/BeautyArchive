...
**Available now:** Salon visit records and photos, hairstyle references and a stylist-facing summary, treatment timing with a one-week notification snooze or a one-time due-date change, and local beauty appointments in a monthly calendar. The calendar also supports cosmetics and perfume purchase plans; marking one purchased adds a single item purchase record. Products support images, per-purchase usage records, replacement estimates and local reminders, and repurchase links. When a purchase URL is provided, B/ONE attempts to fill a missing product image from the page's link metadata after saving; manual image selection remains available. Home brings salon and product actions together and shows registered cosmetics and perfumes with their photos, names, and usage status. Settings provides local notification controls, a JSON export of records and saved photos, and optional Google Calendar account authorization. Connected users can see Google events read-only in the monthly calendar and choose which B/ONE appointments to reflect in their Google primary calendar. Local appointments remain saved if Google sync fails, with a status and retry action. Google authorization, event loading, and write-back have not yet been verified with a real account.
